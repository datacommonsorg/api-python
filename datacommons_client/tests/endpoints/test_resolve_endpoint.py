...
def test_fetch_dcid_by_name(mock_post_request, mock_check_instance_is_valid):
    ...
    response = endpoint.fetch_dcid_by_name(
        names=["Entity1"], entity_type="Place"
    )
...
def test_fetch_dcid_by_wikidata_id(
    ...
    response = endpoint.fetch_dcid_by_wikidata_id(
        wikidata_id="Q12345", entity_type="Country"
    )
...
def test_fetch_dcid_by_coordinates(
    ...
    response = endpoint.fetch_dcid_by_coordinates(
        latitude="37.7749", longitude="-122.4194", entity_type="City"
    )
...
def test_resolve_correspondence_expression():
    ...
    expression = resolve_correspondence_expression(
        from_type="description", to_type="dcid", entity_type="Place"
    )
